...
async def login_for_access_token(
    ...
    logger.info(f"登录请求: {form_data.username} {form_data.password}")
    user = await authenticate_user(db, form_data.username, form_data.password)
    if not user:
        logger.error(f"登录失败: 用户名或密码错误")
        raise HTTPException(
...
async def register(
    ...
    logger.info(f"注册请求: {user.username} {user.email} password: {user.password}")
    ...
    logger.info(f"注册成功: {user.username} {user.email} password: {user.password} model_id: {model_id}")
